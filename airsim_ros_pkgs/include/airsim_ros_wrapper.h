...
// todo move airlib_rpclib typedefs to separate header file?
...
struct VelCmd
{
    ...
    double y;
    double z;
//    msr::airlib_rpclib::DrivetrainType drivetrain;
//    msr::airlib_rpclib::YawMode yaw_mode;
    std::string vehicle_name;

    // VelCmd() : 
    //     x(0), y(0), z(0), 
    //     vehicle_name("") {drivetrain = msr::airlib_rpclib::DrivetrainType::MaxDegreeOfFreedom;
    //             yaw_mode = msr::airlib_rpclib::YawMode();};

    // VelCmd(const double& x, const double& y, const double& z, 
    //         msr::airlib_rpclib::DrivetrainType drivetrain,
    //         const msr::airlib_rpclib::YawMode& yaw_mode,
...
struct GimbalCmd
{
    ...
    // GimbalCmd() : vehicle_name(vehicle_name), camera_name(camera_name), target_quat(msr::airlib_rpclib::Quaternionr(1,0,0,0)) {}

    // GimbalCmd(const std::string& vehicle_name, 
    //         const std::string& camera_name, 
    //         const msr::airlib_rpclib::Quaternionr& target_quat) :
...
//    void gimbal_angle_euler_cmd_cb(const airsim_ros_pkgs::GimbalAngleEulerCmd& gimbal_angle_euler_cmd_msg);
    void car_control_cb(const sensor_msgs::Joy::ConstPtr& Joy);
...
//    bool reset_srv_cb(airsim_ros_pkgs::Reset::Request& request, airsim_ros_pkgs::Reset::Response& response);
    ...
    /// utils. todo parse into an airlib_rpclib<->ROS conversion class
    tf2::Quaternion get_tf2_quat(const msr::airlib::Quaternionr& airlib_rpclib_quat) const;
...
//    nav_msgs::Odometry get_odom_msg_from_airsim_state(const msr::airlib_rpclib::MultirotorState& drone_state) const;
    nav_msgs::Odometry get_odom_msg_from_airsim_state(const CarRpcLibAdapators::CarState& car_state) const;
    airsim_ros_pkgs::GPSYaw get_gps_msg_from_airsim_geo_point(const msr::airlib_rpclib::CarRpcLibAdapators::GeoPoint& geo_point) const;
    sensor_msgs::NavSatFix get_gps_sensor_msg_from_airsim_geo_point(const msr::airlib_rpclib::CarRpcLibAdapators::GeoPoint& geo_point) const;
    ...
    struct CarROS
    {
        ...
        //ros::Subscriber car_control_cb;

//        ros::ServiceServer takeoff_srvr;
//        ros::ServiceServer land_srvr;

        /// State + control
        msr::airlib_rpclib::CarRpcLibAdapators::CarState curr_car_state;
        msr::airlib_rpclib::CarRpcLibAdapators::CarControls car_control;
    ...
    msr::airlib_rpclib::CarRpcLibAdapators::GeoPoint origin_geo_point_;// gps coord of unreal origin
    ...
    //msr::airlib_rpclib::MultirotorRpcLibClient airsim_client_;
    //todo add car

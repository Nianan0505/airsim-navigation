...
//    gimbal_angle_euler_cmd_sub_ = nh_private_.subscribe("gimbal_angle_euler_cmd", 50, &AirsimROSWrapper::gimbal_angle_euler_cmd_cb, this);
    ...
    car_control_sub_ = nh_private_.subscribe("joy", 10, &AirsimROSWrapper::car_control_cb, this);
...
//                                                                                                boost::bind(&AirsimROSWrapper::vel_cmd_world_frame_cb, this, _1, car_ros.vehicle_name));
        ...
        int vehicle_idx = vehicle_name_idx_map_[curr_vehicle_name];
        ROS_INFO_STREAM("curr_vehicle_name = " << curr_vehicle_name<<" and vehicle_idx = "<<vehicle_idx);
...
//    ROS_INFO_STREAM("vehicle_idx"<<vehicle_idx);
...
void AirsimROSWrapper::car_control_cb(const sensor_msgs::Joy::ConstPtr& Joy)
{
   std::lock_guard<std::recursive_mutex> guard(car_control_mutex_);
   const std::string& vehicle_name = "Car1";
   //airsim_client_.enableApiControl(true);
   int vehicle_idx = vehicle_name_idx_map_[vehicle_name];
   float throttle_val = Joy->axes[3]*(-1.0); //throttle
   float steering_val = Joy->axes[2]*(-1.0); //steering
   float manual_gear_val = Joy->axes[5]*1.0;
   bool handbrake_val = Joy->buttons[4];
   bool is_manual_gear_val =  Joy->buttons[5];
   car_ros_vec_[vehicle_idx].car_control.throttle = throttle_val;
   car_ros_vec_[vehicle_idx].car_control.steering = steering_val;
   car_ros_vec_[vehicle_idx].car_control.manual_gear = manual_gear_val;
   car_ros_vec_[vehicle_idx].car_control.handbrake = handbrake_val;
   car_ros_vec_[vehicle_idx].car_control.is_manual_gear = is_manual_gear_val;

   airsim_client_.setCarControls(car_ros_vec_[vehicle_idx].car_control);
   //car_ros_vec_[vehicle_idx].car_control.set_throttle(throttle_val,true);
   ROS_INFO_STREAM("throttle_val: "<<throttle_val << "steering_val: " << steering_val << "manual_gear_val: "<<manual_gear_val <<
                   "handbrake_val: "<<handbrake_val<<"is_manual_gear_val"<<is_manual_gear_val);

}


nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_airsim_state(const CarRpcLibAdapators::CarState& car_state) const
{
    ...
    odom_ned_msg.pose.pose.position.x = car_state.kinematics_estimated.position.x_val;
    odom_ned_msg.pose.pose.position.y = car_state.kinematics_estimated.position.y_val;
    odom_ned_msg.pose.pose.position.z = car_state.kinematics_estimated.position.z_val;
    odom_ned_msg.pose.pose.orientation.x = car_state.kinematics_estimated.orientation.x_val;
    odom_ned_msg.pose.pose.orientation.y = car_state.kinematics_estimated.orientation.y_val;
    odom_ned_msg.pose.pose.orientation.z = car_state.kinematics_estimated.orientation.z_val;
    odom_ned_msg.pose.pose.orientation.w = car_state.kinematics_estimated.orientation.w_val;

    odom_ned_msg.twist.twist.linear.x = car_state.kinematics_estimated.linear_velocity.x_val;
    odom_ned_msg.twist.twist.linear.y = car_state.kinematics_estimated.linear_velocity.y_val;
    odom_ned_msg.twist.twist.linear.z = car_state.kinematics_estimated.linear_velocity.z_val;

    odom_ned_msg.twist.twist.angular.x = car_state.kinematics_estimated.angular_velocity.x_val;
    odom_ned_msg.twist.twist.angular.y = car_state.kinematics_estimated.angular_velocity.y_val;
    odom_ned_msg.twist.twist.angular.z = car_state.kinematics_estimated.angular_velocity.z_val;
...
airsim_ros_pkgs::GPSYaw AirsimROSWrapper::get_gps_msg_from_airsim_geo_point(const msr::airlib_rpclib::CarRpcLibAdapators::GeoPoint& geo_point) const
{
...
sensor_msgs::NavSatFix AirsimROSWrapper::get_gps_sensor_msg_from_airsim_geo_point(const msr::airlib_rpclib::CarRpcLibAdapators::GeoPoint& geo_point) const
{
...
void AirsimROSWrapper::car_state_timer_cb(const ros::TimerEvent& event)
{
    try
    {
        ...
        for (auto& car_ros: car_ros_vec_)
        {
            ...
            //ROS_INFO_STREAM("GPS: "<< car_ros.gps_sensor_msg);
        ...
        if (lidar_pub_vec_.size() > 0)
        {
            ...
            for (const auto& vehicle_lidar_pair: vehicle_lidar_map_)
            {
                ...
                if(lidar_data.point_cloud.size()>2 && lidar_data.time_stamp != prev_timestamp ){
                    ...
                    //ROS_INFO_STREAM("SIZE: "<<lidar_data.point_cloud.size());
                }
                lck.unlock();
                                                                ...
                                                                const std::string frame_id) const
{
    ...
    img_msg_ptr->step = img_response.width * 4; // 4 steps todo un-hardcode. image_width*num_bytes
    img_msg_ptr->header.stamp = curr_ros_time;
    img_msg_ptr->header.frame_id = frame_id;
    img_msg_ptr->height = img_response.height;
    img_msg_ptr->width = img_response.width;
//    std::cout<<"height: "<<img_response.height<<"width: "<<img_response.width<<std::endl;
    img_msg_ptr->encoding = "rgba8"; //the encoding way
    if (is_vulkan_)
    {

...
//多线程参考 https://blog.csdn.net/oiooooio/article/details/44221295
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
{
  ...
  current_time = laserCloudMsg->header.stamp;
  pcl::fromROSMsg(*laserCloudMsg, newScan);
...
void process_1()
{
  ...
  while(!threadQuitFlag)
  {
    ...
    TicToc tt;
    Eigen::Matrix4f _globalPoseDelta = globalPose.globalPose;//暂存全局位置，读取，不用上锁
    Eigen::Matrix4f _globalPose = submap.getPose(temp);
...
void process_2()
{
  ...
  while(!threadQuitFlag)
  {
    ...
    lidarData_cv_mutex.unlock();

    TicToc tt;
    Eigen::Matrix4f _globalPoseDelta = globalPose.globalPose;//暂存全局位置，读取，不用上锁
    Eigen::Matrix4f _globalPose = submap.getPose(temp);//注意 这个pose的计算时间会有问题，要修改策略。
    _globalPoseDelta = _globalPoseDelta.inverse()*_globalPose;
...
{
  using namespace std;
  ros::init(argc, argv, "location_multiThread");
  ros::NodeHandle nh;
  ...
  //nh_param.param<bool>("useImu", useImu, true);
  ...
  subImuLaserPoint = nh.subscribe<sensor_msgs::PointCloud2>(LiDARName, 10, laserCloudHandler);
